...
{	
	...
	while (!(verifie_ligne(x,y,val,S) && verifie_colonne(x,y,val,S) && verifie_colonne(x,y,val,S)))
	{
		val = (val +1)%10;
	}
	return val;	
...
int verifie_bloc (int x, int y, int val, SUDOKU S)
{
	...
	for (i=0; i<3; i++)
	{
		for (j=0; j<3; j++)
		{
			if (i != x && i!= y && S.la_case[j][y].val == val)
			{
				return 0;
			}
			if (i != x && i!= y && S.la_case[x][j].val == val)	
			{
				return 0;
			}	
			if (i != x && i!= y && S.la_case[i][y].val == val)
			{
				return 0;
			}
			if (i != x && i!= y && S.la_case[i][j].val == val)
			{
				return 0;
			}
		}
